...
from config import USED_CONTENT_FIELD,ALPHA_D,LIMIT_D_PATH_LENGTH
...
class Doc_Tree_Object:
      ...
      def __init__(self,article):
          ...
          self.depth2idx={}
          self.size=0
          self.isContentStemmed=True if USED_CONTENT_FIELD.find('stemmed')>-1 else False
          self.used_content_field='stemmed_contents' if self.isContentStemmed==True else 'contents'
          self.T=self.docTree()
      ...
      def initializeNode(self,T,label,depth):
          ...
          T[p]['stemmed_contents' if self.isContentStemmed else 'contents']=[]
      ...
      def traverseAndOutput(self,v,depth,list_section_content):
          ...
          content=remove_stopwords(cleanSentence(temp_line,False,' '),' ')
          if depth>1:
