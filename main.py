...
def computeScore(queryObj,entityObj,structure,lucene_handler,conf_paras):
    ...
    # query verbosity analysis
    #if queryObj.aggregation_node is None
    #if queryObj.aggregation_node is not None:
    ...
    if IS_SAS_USED==True and entityObj.categories is not None:
       ...
       if MODEL_NAME in ['mlm','mlm-tc','mlm-all']:
          text_sim=mlm_sas2(queryObj,entityObj,structure,lucene_handler)
          ...
          text_sim=fsdm_sas2(queryObj,entityObj,structure,lucene_handler)
          if text_sim==0.0 or text_sim==NEGATIVE_INFINITY:
             text_sim=fsdmSim(queryObj,entityObj,lucene_obj)
    ...
    if IS_QUERY_GRAPH_USED==True and len(queryObj.query_predicates)>0:
       query_graph_sim,reachable=scoreQueryGraph(queryObj,entityObj,lucene_handler['entity_uri'],mongoObj)
       #print (score)
       #if reachable==True:
       if query_graph_sim>0:
          w=0.45   
          #if queryObj.aggregation_node is None:
             #w=0.65
          score=w*score+(1-w)*math.log(query_graph_sim)
    return score
...
def handle_process(id_process,queries,RES_STORE_PATH,conf_paras):
    ...
    if IS_WIKI_DOC_TREE_USED==True:
       lucene_handler['wiki']=Lucene_Object(conf_paras.LUCENE_INDEX_WIKI_DIR,'BM25',True,True,structure.mongoObj)
    if IS_SAS_USED==True:
       lucene_handler['category_corpus']=Lucene_Object(conf_paras.LUCENE_INDEX_CATEGORY_CORPUS,'BM25',True,True,structure.mongoObj)
    if IS_QUERY_GRAPH_USED==True or IS_ELR_USED==True:
       lucene_handler['entity_uri']=Lucene_Object(conf_paras.LUCENE_INDEX_URI,'BM25',True,False,structure.mongoObj)
    ...
    REPORT_CAT_FILENAME=os.path.join(RES_STORE_PATH,'report_category_%d.txt'%(id_process))
    ...
    for i in range(len(queries)):
        ...
        # import entities related to query entities
        if IS_QUERY_GRAPH_USED==True: 
            for entity in queryObj.query_entities:
                uri='<http://dbpedia.org/resource/'+entity+'>'
                list_mbpo=structure.mongoObj.conn_mbpo.find({'uri':uri})
                if list_mbpo is None:
                   continue
                for item in list_mbpo:
                    target_en_uri=item['value']
                    d_pair=lucene_obj.findDoc(target_en_uri,'uri',True)
                    d=d_pair[0]
                    if d is None:
                       continue
                    title=d['title']
...
def main(conf_paras):
    ...
    if system_flag=='Windows':
       cmd1='robocopy %s %s *.py'%(r'%cd%',RES_STORE_PATH)
       cmd2='robocopy %s %s *.txt'%(r'%cd%',RES_STORE_PATH)
    else:
       cmd1='cp py3_QGR2/*.py %s'%(RES_STORE_PATH)
       cmd2='cp py3_QGR2/*.txt %s'%(RES_STORE_PATH)
    os.system(cmd1)
    os.system(cmd2)
    # read queries
    queries=[]
   ...
   # begin multiprocessing
    ...
    if WORD_EMBEDDING_TYPE!='NONE':
       delay=60
    elif IS_SAS_USED==True and TAXONOMY=='Wikipedia':
       delay=40
    ...
    cmd='trec_eval -q -m num_q -m num_ret -m num_rel -m num_rel_ret -m P.5,10,20,30,100 -m map_cut.10,100 -m ndcg_cut.10 %s %s > %s'%(PATH_GROUNDTRUTH,res_mp_filepath,cmp_mp_filepath)
    os.system(cmd)
